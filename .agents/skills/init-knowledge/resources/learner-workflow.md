...
> **技能包副本**（随 `init-knowledge` 安装，路径：`.agents/skills/init-knowledge/resources/learner-workflow.md`）。  
> **编辑真源**在 harness 仓 **`.harness/knowledge/learner-workflow.md`**；改真源后请同步复制到本 `resources/` 文件再提交。

**init-knowledge** 阶段 **A0** 或 **learn** 前置检查会将副本写入消费端 **`$TARGET/.harness/knowledge/learner-workflow.md`**（若缺失）。执行学习前须 **Read** 目标仓中的该文件（非直接读本 `resources/` 路径）。
