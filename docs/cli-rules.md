...
在未使用 `--to` 时，`rules add` / `experimental_install` 需要确定「哪一个仓库」作为项目根（写入 `rules-lock.json`、以及 `.cursor/rules` 等相对路径）。

解析顺序：

1. **`--cwd <dir>`** 或 **`-C <dir>`**（显式指定）
2. **`INIT_CWD`**：pnpm / npm 在子进程里常设置为**发起命令时**的工作目录；用 `pnpm --dir path/to/cli dev rules add ...` 时，Node 的 `process.cwd()` 可能是 `cli` 包目录，但 `INIT_CWD` 仍可为你的业务仓库根，从而把规则装对位置。
3. **`process.cwd()`**（兜底）

若仍落到错误的目录（例如 `cli/.cursor`），请使用 **`--cwd D:\your-repo`**。
...
- **目标项目不是 harness 时**：在 **目标仓库根**打开终端（如 `cd D:\one-eino`），再执行  
  `skills rules add <来源> -a cursor|claude-code`，规则会写入**目标项目根**下的 `.cursor/rules` / `.claude/rules`，并在该目录生成 **`rules-lock.json`**（项目根见上文「目标项目根」；`pnpm --dir …/cli` 场景通常可用 `INIT_CWD`，否则加 `--cwd`）。  
