...
func NewStreamHandler(exec *application.Executor, rl streamRateLimiter, streamPerMinute int) app.HandlerFunc {
	if streamPerMinute <= 0 {
		streamPerMinute = defaultStreamRateLimitPerMinute
	}
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if !checkStreamRateLimit(ctx, c, rl, streamPerMinute) {
			return
		}

		st := exec.Store
		sessionID, sess, err := application.EnsureSession(ctx, st, req.SessionID, userID, req.Mode)
		if err != nil {
		...
		incoming := 2
		if sess != nil && sess.MessageCount()+incoming > domain.MaxSessionMessages {
		...
		in := application.Input{UserID: userID, Message: req.Message, Mode: req.Mode}
		...
		if err := application.PersistTurn(ctx, st, sessionID, userID, gate, req.Message, fullChat); err != nil {
			if errors.Is(err, domain.ErrSessionMessageCap) {
				writeGateError(w, "session_message_cap", "????????????50 ?????????????????")
				return
			}
			writeProviderError(w, exec, err)
			return
...
func handleGateTurn(
	...
	if outcome.Crisis != nil {
		...
		_ = application.PersistTurn(ctx, st, sessionID, userID, gate, userMessage, outcome.Crisis.Body)
		writeTemplateEvent(w, "crisis", outcome.Crisis.TemplateID, outcome.Crisis.Body)
		return
	}
	if outcome.Medical != nil {
		safety.Audit(sessionID, gate, userMessage)
		_ = application.PersistTurn(ctx, st, sessionID, userID, gate, userMessage, outcome.Medical.Body)
		writeTemplateEvent(w, "medical", outcome.Medical.TemplateID, outcome.Medical.Body)
		return
	}
	if outcome.Block != nil {
		safety.Audit(sessionID, gate, userMessage)
		_ = application.PersistTurn(ctx, st, sessionID, userID, gate, userMessage, "")
...
func writeDone(w *sse.Writer, sessionID string, st domain.Store, ctx context.Context, userID string) {
	count := 0
...
func writeProviderError(w *sse.Writer, exec *application.Executor, err error) {
