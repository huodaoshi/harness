...
func TestProfileInject_DistressMode(t *testing.T) {
	ctx := context.Background()
	mem := store.NewMemoryStore()
	_ = mem.UpsertProfile(ctx, domain.RelationshipProfile{
...
type injectCase struct {
	...
	message        string
	mode           string
	profile        *domain.RelationshipProfile
...
func buildInjectCases() []injectCase {
	return []injectCase{
		{name: "01_profile_issue_only", userID: "u01", message: "我又想他了", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u01", CurrentIssue: "和伴侣反复冷战", Self: domain.ProfileSelf{Note: "容易焦虑"},
		}, mustContain: []string{"和伴侣反复冷战", "容易焦虑"}},
		{name: "02_summary_only", userID: "u02", message: "继续昨晚的话题", mode: "normal", summary: &domain.SessionSummary{
			UserID: "u02", Summary3: []string{"昨晚谈到母亲边界", "情绪一度失控", "最后稍微平静"},
		}, mustContain: []string{"昨晚谈到母亲边界", "情绪一度失控"}},
		{name: "03_profile_and_summary", userID: "u03", message: "还是很难受", mode: "normal", profile: &domain.RelationshipProfile{
		...
		{name: "05_person_label", userID: "u05", message: "今天他又这样", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u05", People: []domain.ProfilePerson{{Label: "阿明", Relation: "伴侣", Note: "回避型"}},
			CurrentIssue: "沟通僵局",
		}, mustContain: []string{"阿明", "回避型", "沟通僵局"}},
		{name: "06_two_people", userID: "u06", message: "夹在中间", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u06", People: []domain.ProfilePerson{
				{Label: "妈妈", Relation: "母亲"},
				{Label: "老公", Relation: "伴侣"},
			},
		}, mustContain: []string{"妈妈", "老公"}},
		{name: "07_self_note", userID: "u07", message: "很累", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u07", Self: domain.ProfileSelf{Note: "长期失眠"},
		...
		{name: "09_profile_distress_issue", userID: "u09b", message: "撑不住", mode: "distress", profile: &domain.RelationshipProfile{
			UserID: "u09b", CurrentIssue: "催婚压力",
		}, mustContain: []string{"催婚压力"}},
		{name: "10_long_issue_snippet", userID: "u10", message: "怎么办", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u10", CurrentIssue: "婆婆同住矛盾升级",
		}, mustContain: []string{"婆婆同住"}},
		{name: "11_summary_line2", userID: "u11", message: "嗯", mode: "normal", summary: &domain.SessionSummary{
			UserID: "u11", Summary3: []string{"第一句", "第二句关键词", "第三句"},
		}, mustContain: []string{"第二句关键词"}},
		{name: "12_profile_without_summary", userID: "u12", message: "聊几句", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u12", CurrentIssue: "异地信任问题",
		}, mustNotContain: []string{"[上次会话摘要]"}, mustContain: []string{"异地信任"}},
		{name: "13_summary_without_profile", userID: "u13", message: "继续", mode: "normal", summary: &domain.SessionSummary{
		...
		{name: "15_partner_conflict", userID: "u15", message: "他又冷暴力", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u15", People: []domain.ProfilePerson{{Label: "小陈", Relation: "伴侣"}}, CurrentIssue: "冷暴力",
		}, mustContain: []string{"小陈", "冷暴力"}},
		{name: "16_parent_boundary", userID: "u16", message: "回家就吵", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u16", CurrentIssue: "父母过度干涉",
		}, summary: &domain.SessionSummary{UserID: "u16", Summary3: []string{"谈到经济独立"}},
			mustContain: []string{"父母过度干涉", "经济独立"}},
		{name: "17_three_line_summary", userID: "u17", message: "在吗", mode: "normal", summary: &domain.SessionSummary{
			UserID: "u17", Summary3: []string{"A", "B", "C"},
		}, mustContain: []string{"A", "B", "C"}},
		{name: "18_self_and_issue", userID: "u18", message: "我又哭了", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u18", Self: domain.ProfileSelf{Note: "高敏感"}, CurrentIssue: "分手未愈",
		}, mustContain: []string{"高敏感", "分手未愈"}},
		{name: "19_work_family", userID: "u19", message: "两边压力", mode: "normal", profile: &domain.RelationshipProfile{
			UserID: "u19", CurrentIssue: "工作与家庭拉扯", Self: domain.ProfileSelf{Note: "职场新人"},
		}, mustContain: []string{"工作与家庭拉扯", "职场新人"}},
		{name: "20_inject_marker", userID: "u20", message: "test", mode: "normal", profile: &domain.RelationshipProfile{
...
func seedStore(t *testing.T, ctx context.Context, mem *store.MemoryStore, tc injectCase) {
	...
	if tc.name == "08_latest_summary_wins" {
		_ = mem.SaveSummary(ctx, domain.SessionSummary{
