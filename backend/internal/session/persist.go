...
func EnsureSession(ctx context.Context, st store.Store, sessionID, userID, mode string) (string, *store.StoredSession, error) {
	if sessionID != "" {
...
func PersistTurn(
	...
	if assistantText != "" {
		msgs = append(msgs, store.SessionMessage{Role: "assistant", Content: assistantText})
	}
...
func FinalizeSession(ctx context.Context, st store.Store, sessionID, userID string) ([]string, error) {
	sess, err := st.GetSession(ctx, sessionID)
	if err != nil {
		return nil, err
	}
