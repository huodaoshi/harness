...
type profilePutBody struct {
	Self         store.ProfileSelf     `json:"self"`
	People       []store.ProfilePerson `json:"people"`
	CurrentIssue string                `json:"current_issue"`
}

// NewGetProfileHandler returns GET /v1/profile for the authenticated user_id.
func NewGetProfileHandler(st store.Store) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if p == nil {
			c.JSON(consts.StatusOK, store.EmptyProfile(userID))
			return
		}
...
func NewPutProfileHandler(st store.Store) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		userID, ok := requireUserID(c)
		if !ok {
			return
		...
		if body.People == nil {
			body.People = []store.ProfilePerson{}
		}

		p := store.RelationshipProfile{
			UserID:       userID,
			Self:         body.Self,
			People:       body.People,
