...
- [快速开始](#快速开始)
- [在其它项目中使用](#在其它项目中使用)
  - [用 node 直接调用 CLI](#用-node-直接调用-cli)
- [Agent 工作流](#agent-工作流)
- [团队技能一览](#团队技能一览)
- [知识体系（三层）](#知识体系三层)
- [CLI 开发](#cli-开发)
...
若本机 **`pnpm` 版本与 `cli/package.json` 的 `packageManager` 不一致**（corepack 报错），可改用下文 **[用 node 直接调用 CLI](#用-node-直接调用-cli)**，无需先对齐 pnpm 版本。
...
`rules add` 与技能安装共用同一入口 **`cli/src/cli.ts`**，可用 **`node`** 直接执行，与 `pnpm --dir cli dev …` 等价。适合 **pnpm 版本冲突**、**未在 harness/cli 下 `pnpm install`**、或脚本里写死绝对路径的场景。

将 **`HARNESS`**、**`APP`** 换成你本机路径（示例：`D:\harness`、`D:\jw\jw-ms-client`）。
...
# ── 技能（子命令 add，不是 rules add）────────────────────────────

# 列出 harness 里可安装的技能
node $CLI add $HARNESS --list --cwd $APP

# 安装全部团队技能到目标项目（Cursor）
node $CLI add $HARNESS --all -y -a cursor --cwd $APP

# 只装指定技能
node $CLI add $HARNESS --skill init-knowledge --skill learn --skill triage -y -a cursor --cwd $APP

# 查看目标项目已装技能
node $CLI list --cwd $APP

# ── 规则（子命令 rules add）──────────────────────────────────────

node $CLI rules add $HARNESS -a cursor -y --cwd $APP
node $CLI rules add $HARNESS -a claude-code -y --cwd $APP
```

| 操作 | 子命令 | 安装结果（项目级） |
|------|--------|-------------------|
| 技能 | `add <包路径>` | `.agents/skills/<name>/` + `skills-lock.json` |
| 规则 | `rules add <包路径> -a cursor\|claude-code` | `.cursor/rules/` 或 `.claude/rules/` + `rules-lock.json` |

**`--cwd`**：不在目标项目目录里执行命令时**必须**加上，否则可能装到错误目录。

**`-a cursor`**：指定 Cursor；省略时 CLI 会按当前环境自动检测。

**`-y`**：跳过交互确认。

等价写法（需在 harness 根且 pnpm 可用时）：

```powershell
pnpm --dir D:\harness\cli dev add D:\harness --all -y -a cursor --cwd D:\your-app
pnpm --dir D:\harness\cli dev rules add D:\harness -a cursor -y --cwd D:\your-app
```
