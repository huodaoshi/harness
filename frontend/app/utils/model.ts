...
export function normalizeServiceProvider(providerName?: string) {
  if (!providerName) return providerName;
  const id = providerName.toLowerCase();
  if (id === "bytedance") return ServiceProvider.ByteDance;
  return providerName;
}

const customProvider = (providerName: string) => {
  const id = providerName.toLowerCase();
  const normalized = normalizeServiceProvider(providerName) ?? providerName;
  return {
    id,
    providerName: normalized,
    providerType: id === "bytedance" ? "bytedance" : "custom",
...
export function isModelNotavailableInServer(
  ...
  for (const providerName of providerNamesArray) {
    // ByteDance: CUSTOM_MODELS swaps displayName (e.g. doubao-seed) vs API name (ep-xxx)
    if (providerName === ServiceProvider.ByteDance) {
      const entry = Object.values(modelTable).find(
        (v) =>
          (v.provider?.providerName === ServiceProvider.ByteDance ||
            v.provider?.id === "bytedance") &&
          (v.name === modelName || v.displayName === modelName),
      );
...
/** Map UI / session model id to Volcengine endpoint id (ep-...) when using CUSTOM_MODELS. */
export function resolveByteDanceModelName(
  customModels: string,
  modelName: string,
): string | undefined {
  if (!customModels || !modelName) return undefined;
  const modelTable = collectModelTable(DEFAULT_MODELS, customModels);
  const entry = Object.values(modelTable).find(
    (v) =>
      (v.provider?.providerName === ServiceProvider.ByteDance ||
